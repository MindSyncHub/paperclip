...
describeEmbeddedPostgres("tool access service", () => {
  ...
  it("initializes stateful Streamable HTTP servers and remembers that future calls need a session", async () => {
    ...
    // The handshake leads now, so the session-less probe never happens.
    expect(requests[0]).toEqual({ method: "initialize", sessionId: null });
    expect(requests.some(({ method }) => method === "tools/list")).toBe(true);
  ...
  it("connects a server that refuses every message sent before initialize", async () => {
    const company = await createCompany(db);
    const service = createTestToolAccessService(db);
    const requests: Array<{ method: string; sessionId: string | null }> = [];
    vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
      ...
      if (payload.method === "tools/list" && !requestHeaders.get("mcp-session-id")) {
        // nanobot, which fronts Obot MCP gateways, refuses anything sent before
        // `initialize` with 405 rather than 400 or 404 (#12697).
    ...
    // The ordering keeps the 405 from happening at all: no status is matched.
    expect(requests[0]).toEqual({ method: "initialize", sessionId: null });
    expect(requests.some(({ method, sessionId }) => method === "tools/list" && !sessionId)).toBe(false);
  });

  it("remembers that a stateless server needs no session and stops handshaking", async () => {
    const company = await createCompany(db);
    const service = createTestToolAccessService(db);
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
      ...
      if (payload.method === "initialize") {
        // A stateless server accepts the handshake but issues no session id.
        return new Response(JSON.stringify({
          jsonrpc: "2.0",
          id: payload.id,
          result: {
    ...
    const connected = await service.connectGalleryApp(company.id, {
      link: "https://stateless.example/mcp",
      name: "Stateless MCP",
    }, { actorType: "user", actorId: "board" });

    expect(connected.connection.config).toMatchObject({ mcpSessionRequired: false });
    const handshakesAfterConnect = methods.filter((method) => method === "initialize").length;

    await service.refreshCatalog(connected.connectionId, { actorType: "user", actorId: "board" });

    // The stored answer holds, so the next refresh is a single request.
    expect(methods.filter((method) => method === "initialize").length).toBe(handshakesAfterConnect);
  });

  it("recovers when a server that needed no session starts requiring one", async () => {
    const company = await createCompany(db);
    const service = createTestToolAccessService(db);
      ...
      if (payload.method === "initialize") {
        return new Response(JSON.stringify({
          ...
          result: {
            protocolVersion: "2025-06-18",
            capabilities: { tools: {} },
            serverInfo: { name: "upgraded", version: "1" },
          },
        }), {
          status: 200,
          headers: {
            "content-type": "application/json",
            ...(sessionRequired ? { "mcp-session-id": "session-upgraded" } : {}),
          },
        });
      }
      if (payload.method === "notifications/initialized") return new Response(null, { status: 202 });
    ...
    const connected = await service.connectGalleryApp(company.id, {
      link: "https://upgraded.example/mcp",
      name: "Upgraded MCP",
    }, { actorType: "user", actorId: "board" });
    expect(connected.connection.config).toMatchObject({ mcpSessionRequired: false });

    // The server starts keeping sessions. The stored `false` sends the request
    // session-less first, and the failure alone is enough to handshake again.
  ...
  it("starts and completes OAuth app sign-in with PKCE state and secret-backed tokens", async () => {
    ...
    // Discovery opens the MCP session first, so each probe is initialize +
    // notifications/initialized + the request itself.
